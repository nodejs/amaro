...
/// Deserialize the JS options object shared by every binding, falling back to
/// the defaults when nothing (or a falsy value) is provided.
...
/// Coerce the JS input into a UTF-8 string, accepting either a string or a
/// `Uint8Array` as every binding does.
...
fn parse_program(input: String, options: Options) -> Result<Program, Vec<JsonDiagnostic>> {
    ...
    try_with_json_handler(cm.clone(), |handler| {
        ...
        // A fatal parse error aborts before any AST is produced; emit it as an
        // `InvalidSyntax` diagnostic so it surfaces through the shared handler.
        ...
        // Recoverable errors still yield an AST, but we report them the same way
        // `operate` does; `try_with_json_handler` turns any emitted diagnostic
